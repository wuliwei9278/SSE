...
void BPREngine::update(const PosNegTriplet& triplet) {
  size_t uidx = triplet.userIdx;
  float r = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
  if (r > config_.user_threshold) {
    //uidx = static_cast <size_t> (rand() % 3937);
    uidx = static_cast <size_t> (rand() % userIndex_.size());
  }
  size_t pidx = triplet.posItemIdx;
  r = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
  if (r > config_.item_threshold) {
    pidx = static_cast <size_t> (rand() % itemIndex_.size());
  }
